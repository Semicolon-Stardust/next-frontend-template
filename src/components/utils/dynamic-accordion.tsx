		...
		<Accordion type="single" collapsible>
			{items.map((item) => (
				<AccordionItem key={item.id} value={item.id}>
					<AccordionTrigger className="flex items-center justify-between py-4 text-lg font-medium text-gray-900 transition-colors duration-300 hover:text-gray-700">
						{item.title}
					</AccordionTrigger>
